...
class ProxyTest(KcoinTestFramework):        
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: kcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        ...
        if test_onion:
            ...
            node.addnode("kcoinostk4e4re.onion:29444", "onetry")
            cmd = proxies[2].queue.get()
            assert(isinstance(cmd, Socks5Command))
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, "kcoinostk4e4re.onion")
            assert_equal(cmd.port, 29444)
            if not auth:
                assert_equal(cmd.username, None)
                assert_equal(cmd.password, None)
            rv.append(cmd)
        ...
        node.addnode("node.noumenon:29444", "onetry")
