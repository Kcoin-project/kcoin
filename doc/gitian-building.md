...
*Setup instructions for a gitian build of Kcoin using a Debian VM or physical system.*
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-kcoin-host/README.md).
...
Clone the git repositories for kcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kcoin/kcoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the kcoin repository
...
To build Kcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the kcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/kcoin/kcoin
...
For example:
```bash
URL=https://github.com/laanwj/kcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kcoin=${COMMIT} --url kcoin=${URL} ../kcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kcoin=${COMMIT} --url kcoin=${URL} ../kcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kcoin=${COMMIT} --url kcoin=${URL} ../kcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kcoin-osx-build.assert
```
...
[kcoin/gitian.sigs](https://github.com/kcoin/gitian.sigs/) repository, or if that's not possible create a pull
