...
###   This script attempts to download the signature file SHA256SUMS.asc from kcoin.org
...
WORKINGDIR="/tmp/kcoin"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://kcoin.org/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kcoin.org/bin/"
VERSIONPREFIX="kcoin-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kcoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[kcoin-]<version>-[rc[0-9]] (example: kcoin-0.9.2-rc1)"
   echo "wget output:"
